locals {
  ...
  postgresql_username = coalesce(var.env_postgresql_username, random_pet.pg-user-fallback.id)
  postgresql_password = coalesce(var.env_postgresql_password, one(digitalocean_database_user.postgres-app[*].password))
